...
-- Anti Lag + Fake Win Added To Visuals
...
visuals:create_module({
    title = "Anti Lag",
    flag = "AntiLag",
    description = "Reduces lag by removing visual effects",
    section = "left",
    callback = function(state)
        if state then
            ...
            for O, mat in pairs(originalMaterials) do
                if O and O.Parent then
                    O.Material = mat
                end
            end

            for _, d in pairs(originalDecalsTextures) do
                if d.Object and d.Parent then
...
local function getIconImage(num)
    return num >= 10 and iconb or icono
end

local function getDisplay()
    local folders = {"Dead", "Alive"}
...
local function clearFake()
    ...
    if char then
        local old = char:FindFirstChild("FakeStreakBillboard")
...
local function applyFake(num)
    if not groupEnabled or num < 1 then
        clearFake()
        return
    end
    ...
    if not char or not display then
        return
    end
    ...
    local main = Instance.new("Frame", bb)
    ...
    icon.Position = UDim2.new(0.5, 0, 0.9, 0)
    icon.AnchorPoint = Vector2.new(0.5, 1)
    icon.Image = getIconImage(num)

    local label = Instance.new("TextLabel", main)
    label.BackgroundTransparency = 1
    label.Size = UDim2.new(1.5, 0, 0.8, 0)
...
local fakeWinModule = visuals:create_module({
    title = "Fake Win",
    flag = "FakeWin",
    description = "Shows fake winstreak",
    section = "left",
    callback = function(state)
        groupEnabled = state

        if state then
            applyFake(fakeStreakValue)
        else
            clearFake()
        end
    end
...
fakeWinModule:create_slider({
    title = "Fake Streak Value",
    flag = "FakeStreakValue",
    min = 1,
    max = 999999,
    increment = 1,
    default = 1,
    callback = function(value)
        fakeStreakValue = value
