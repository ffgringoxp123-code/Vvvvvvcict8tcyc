...
-- Auto Parry V2
getgenv().AutoParryV2Enabled = false

local AutoParryV2Module = rage:create_module({
    title = "Auto Parry V2",
    flag = "Auto_Parry_V2",
    description = "Advanced auto parry system",
    section = "left",
    callback = function(state)
        getgenv().AutoParryV2Enabled = state

        if state and not getgenv()._AutoParryV2Loaded then
            getgenv()._AutoParryV2Loaded = true
            ...
            task.spawn(function()
                ...
                do
                    local success, result = pcall(function()
                        return loadstring(game:HttpGet('https://raw.githubusercontent.com/Lose1s/Module_Wrapper/refs/heads/main/Parry_Animation.lua'))()
                    end)
                    if success then
                        Parry_Animation_API = result
                    end
                end
                ...
                local statsPingItem = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]
                local RunService = game:GetService("RunService")
                ...
                local function InitBallState(ball)
                    if ball:GetAttribute("realBall") then
                        ballStates[ball] = { parried = false }

                        ball:GetAttributeChangedSignal("target"):Connect(function()
                            if ballStates[ball] then
                                ballStates[ball].parried = false
                            end
                        end)
                    end
                ...
                BallsFolder.ChildAdded:Connect(InitBallState)
                ...
                local function ExecuteParry()
                    ...
                    if Parry_Animation_API then
                        pcall(function()
                            if type(Parry_Animation_API) == "function" then
                                Parry_Animation_API()
                            elseif type(Parry_Animation_API) == "table" and Parry_Animation_API.Play then
                                Parry_Animation_API:Play()
                            end
                        end)
                    end
                end
                ...
                local function AutoParryLoop()
                    if not getgenv().AutoParryV2Enabled then
                        return
                    end
                    ...
                    local pingMs = statsPingItem:GetValue()
                    ...
                    for _, ball in ipairs(BallsFolder:GetChildren()) do
                        ...
                        local speed = velVec.Magnitude

                        local parryRadius = pingFactor + math.max(speed / (2.4 * ACCURACY_FACTOR), 9.5)
                ...
                autoParryConnection = RunService.Stepped:Connect(function()
                    if getgenv().AutoParryV2Enabled then
                        AutoParryLoop()
                    elseif autoParryConnection then
