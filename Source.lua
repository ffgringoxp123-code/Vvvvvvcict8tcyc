    ...
    do
        ...
        Library.Window = function(Self, Params)
            ...
            local Items = { } do 
                ...
                -- Find this section in the Window function (around line 650-700) and replace it:
...
-- Find this section in the Window function and replace the Icon and Title sections:

-- Replace the Icon section (moved further left):
Items["Icon"] = Library:Create("ImageLabel", {
    ...
    Image = Window.Logo,
    BackgroundTransparency = 1,
    Position = UDim2.new(0, 15, 0.5, 0),  -- Changed from 30 to 15 (further left)
    ZIndex = 2,
    BorderSizePixel = 0
...
-- Replace Title (moved to be right next to the icon):
Items["Title"] = Library:Create("TextLabel", {
    ...
    BackgroundTransparency = 1,
    Position = UDim2.new(0, 65, 0.5, -8),  -- Changed from 80 to 65 (closer to icon)
    BorderSizePixel = 0,
    AutomaticSize = Enum.AutomaticSize.X,
    TextXAlignment = Enum.TextXAlignment.Left
}):AddToTheme({TextColor3 = 'Accent'})

-- Replace SubTitle (adjusted position):
Items["SubTitle"] = Library:Create("TextLabel", {
    ...
    BackgroundTransparency = 1,
    Position = UDim2.new(0, 67, 0.5, 12),  -- Changed from 82 to 67
    BorderSizePixel = 0,
    AutomaticSize = Enum.AutomaticSize.X,
    TextXAlignment = Enum.TextXAlignment.Left
})

-- Minimize hitbox (transparent, sits over the icon like SKR UI's Minimize button)
Items["Minimize"] = Library:Create("TextButton", {
    Name = "\0",
    Parent = Items["Top"].Instance,
    Text = "",
    AutoButtonColor = false,
...
Items["Minimize"]:Connect("MouseButton1Click", function()
    Window:SetOpen(not Window.IsOpen)
            ...
            function Window:SetOpen(Bool)
                ...
                if Bool then
                    TargetSize = Window.FullSize or Items["MainFrame"].Instance.Size
                else
                    Window.FullSize = Items["MainFrame"].Instance.Size

                    local TitleBoundsParams = Instance.new("GetTextBoundsParams")
                    TitleBoundsParams.Text = Window.Name
                    TitleBoundsParams.Font = Library.Font
                    TitleBoundsParams.Size = 24
                    TitleBoundsParams.Width = 1000

                    local TitleBounds = TextService:GetTextBoundsAsync(TitleBoundsParams)

                    local MinimizedWidth = math.max(160, Items["Title"].Instance.Position.X.Offset + TitleBounds.X + 35)
                    TargetSize = UDim2.new(0, MinimizedWidth, 0, 60)
                end

                local NewTween = Items["MainFrame"]:Tween({
                    Size = TargetSize
