        ...
        if state then
            for _, obj in ipairs(workspace:GetDescendants()) do
                ...
                elseif obj:IsA("Decal") or obj:IsA("Texture") then
                    table.insert(originalDecalsTextures, {
                        Object = obj,
                        Parent = obj.Parent
                    })
...
local function removeFake()
    local folders = {"Alive","Dead"}

    for _, folderName in pairs(folders) do
        local folder = workspace:FindFirstChild(folderName)

        if folder and folder:FindFirstChild(game.Players.LocalPlayer.Name) then
            local char = folder[game.Players.LocalPlayer.Name]

            local old = char:FindFirstChild("FakeWinGui")
...
local function createFake(value)
    removeFake()
...
local fakeModule = visuals:create_module({
    title = "Fake Win",
    flag = "FakeWin",
    description = "Fake winstreak display",
    section = "left",
    callback = function(state)
        fakeEnabled = state

        if state then
            createFake(fakeValue)
        else
            removeFake()
        end
    end
